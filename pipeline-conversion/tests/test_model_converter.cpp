...
TEST(ModelConverterTest, ThrowsOnNonExistentFile) {
    EXPECT_THROW(
        ModelConverter("non_existent_file.inp", "EPAnet"),
        std::runtime_error
    );
}
...
TEST(ModelConverterTest, ThrowsOnUnsupportedInputType) {
    ...
    const auto test_file = std::filesystem::path(__FILE__).parent_path().parent_path() / "test_data" / "test_model.inp";
    if (std::filesystem::exists(test_file)) {
        EXPECT_THROW(
            ModelConverter(test_file.string(), "UnsupportedFormat"),
            std::runtime_error
        );
    }
}
...
TEST(ModelConverterTest, ConvertThrowsIfOutputFileExists) {
    const auto test_data = std::filesystem::path(__FILE__).parent_path().parent_path() / "test_data";
    ...
    if (std::filesystem::exists(input_file)) {
        ...
        EXPECT_THROW(
            converter.convert(existing_output.string(), "WANDA"),
            std::runtime_error
        );
    }
}
