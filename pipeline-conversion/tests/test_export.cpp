...
Network create_sample_network() {
    Network network;
    network.nodes.push_back({.id = "N1", .elevation = 10.0, .position = {0.0f, 0.0f}});
    network.nodes.push_back({.id = "N2", .elevation = 5.0, .position = {100.0f, 0.0f}});
    network.nodes.push_back({.id = "N3", .elevation = 3.0, .position = {200.0f, 0.0f}});

    network.pipes.push_back({
        .name = "P1",
        .length = 500.0,
        .inner_diameter = 0.3,
...
protected:
    ...
    bool wanda_available() const {
        return std::filesystem::exists(R"(c:\Program Files (x86)\Deltares\Wanda 4.8\Bin64\)");
    }
};

TEST_F(WandaExportTest, ExporterCreatesOutputFile) {
    if (!wanda_available()) GTEST_SKIP() << "WANDA binaries not available";
...
protected:
    ...
    bool inputs_available() const {
        return std::filesystem::exists(epanet_input) &&
               std::filesystem::exists(R"(c:\Program Files (x86)\Deltares\Wanda 4.8\Bin64\)");
    }
};

TEST_F(ModelConverterExportTest, EpanetToWandaProducesFile) {
    if (!inputs_available()) GTEST_SKIP() << "EPAnet input or WANDA binaries not available";
