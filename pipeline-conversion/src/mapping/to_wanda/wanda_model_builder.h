...
/**
 * @brief Represents a pending connection between a pipe and a node in the WANDA
 * model.
 *
 * Fields:
 * - pipe_id           : Reference to the WANDA pipe component.
 * - node_id           : Identifier of the node to connect.
 * - connection_point  : Connection point index on the pipe (1 = upstream, 2 =
 * downstream).
 */
struct Connection {
...
/**
 * @brief Builds a WANDA model by adding nodes and pipes, then finalizing
 * connections.
 *
 ...
 * - finalize():     Connects all pipes to their respective nodes and saves the
 * model.
 */
class WandaModelBuilder {
  public:
    ...
    ~WandaModelBuilder() { model_.close(); }
